...
int main(int argc, char** argv)
{
    ...
    strcpy_s(entries[2].value, "C:\\Images\\original.png");
    strcpy_s(entries[2].defaultValue, "C:\\Images\\default.png");
    if (validate(&header, entries) != CHUI_STATUS_OK) return 31;
    const LONG pathInstance = openDialog(owner, &header, entries, notification);
    ...
    const char* updatedPath =
        ...
        "version-2 entry value is not limited by the former 128-byte field\\"
    ...
    SendMessageW(dialog, WM_COMMAND, MAKEWPARAM(104, BN_CLICKED),
        reinterpret_cast<LPARAM>(applyButton));
    if (!consume(notification, &completedInstance, &result) ||
        result != CHUI_RESULT_APPLY || std::strcmp(entries[2].value, updatedPath) ||
