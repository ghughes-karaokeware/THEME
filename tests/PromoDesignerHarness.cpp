...
int main(int argc,char** argv)
{
    ...
    CHPT_PROMO_DATA data{};data.version=0x00010000;data.structureSize=sizeof(data);data.bufferCapacity=sizeof(data.text);data.maximumLength=1027;data.flags=CHPT_FLAG_AUTOLOAD;strcpy_s(data.text,"Welcome to <3><B>Friday Night Karaoke</B><0>!\r\nPlain trailer");
    LONG instance=open(owner,&data,notify);if(instance<=0)return 5;HWND dialog=FindWindowW(L"CHThemePromoDesigner",nullptr);if(!dialog)return 6;SendMessageW(dialog,WM_COMMAND,120,0);if(!PumpUntilClosed(dialog))return 7;DWORD completed{};LONG result{};if(!consume(notify,&completed,&result)||completed!=static_cast<DWORD>(instance)||result!=CHPT_RESULT_OK)return 8;if(!strstr(data.text,"Friday Night Karaoke")||!strstr(data.text,"Plain trailer")||!(data.flags&CHPT_FLAG_AUTOLOAD))return 9;
