...
LRESULT CALLBACK SliderWindowProc(HWND window, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_TIMER:
        if (wParam == 1) {
            // Clarion performs several parent/sibling paints while the window
            // is opening. Reassert the layered child briefly so those paints
            // cannot leave the slider concealed until the first mouse move.
            SetWindowPos(window, HWND_TOP, 0, 0, 0, 0,
                SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE | SWP_SHOWWINDOW);
            RedrawWindow(window, nullptr, nullptr,
                RDW_INVALIDATE | RDW_UPDATENOW | RDW_NOERASE);
            if (++data->initialPaintPasses >= 6)
                KillTimer(window, 1);
            return 0;
        }
        break;
    ...
    int orientation, int minimum, int maximum, int value, HWND notifyButton)
{
    ...
    RedrawWindow(slider, nullptr, nullptr,
        RDW_INVALIDATE | RDW_ERASE | RDW_UPDATENOW | RDW_FRAME);
    // The parent continues initializing after CHSlider_Create returns. A
    // short-lived timer makes the first paint as reliable as hover repainting.
    SetTimer(slider, 1, 40, nullptr);
    return slider;
}
